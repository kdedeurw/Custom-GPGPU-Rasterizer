...
RGBColor Texture::SampleLinear(const FVector2& uv) const
{
	...
	pixels[0] = float(x - 1 + (y * m_pSurface->w)); //original pixel - 1x
	if (pixels[0] < 0)
		pixels[0] = 0;
	//possible issue: x might shove back from left to right - 1y
	pixels[1] = float(x + 1 + (y * m_pSurface->w)); //original pixel + 1x
	if (pixels[1] < max)
		pixels[1] = max;
	//possible issue: x might shove back from right to left + 1y
	pixels[2] = float(x + ((y + 1) * m_pSurface->w)); //original pixel + 1y
	if (pixels[2] < max)
		pixels[2] = max;
	pixels[3] = float(x + ((y - 1) * m_pSurface->w)); //original pixel - 1y
	if (pixels[3] < 0)
		pixels[3] = 0;
