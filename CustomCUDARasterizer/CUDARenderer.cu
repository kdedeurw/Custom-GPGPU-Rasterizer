...
CPU_CALLABLE void CUDARenderer::InitCUDARasterizer()
{
	...
	*/
	...
	size = sizeof(*m_WindowHelper.h_BackBufferPixels);
	CheckErrorCuda(cudaMallocHost((void**)&m_WindowHelper.h_BackBufferPixels, width * height * size));
	//CheckErrorCuda(cudaHostAlloc((void**)&m_WindowHelper.h_BackBufferPixels, width * height * size, cudaHostAllocPortable));
	...
	*/
	...
	//size = sizeof(*m_WindowHelper.pBackBufferPixels);
...
CPU_CALLABLE void CUDARenderer::CopyMeshBuffers(const std::vector<IVertex>& vertexBuffer, const std::vector<unsigned int>& indexBuffer, int meshIdx)
{
	...
	CheckErrorCuda(cudaMemcpy(dev_IVertexBuffer[meshIdx], &vertexBuffer[0], vertexBuffer.size() * sizeof(vertexBuffer[0]), cudaMemcpyHostToDevice));
	//Copy Index Buffer
	CheckErrorCuda(cudaMemcpy(dev_IndexBuffer[meshIdx], &indexBuffer[0], indexBuffer.size() * sizeof(indexBuffer[0]), cudaMemcpyHostToDevice));
}
...
CPU_CALLABLE void CUDARenderer::FreeCUDARasterizer()
{
	...
	CheckErrorCuda(cudaFreeHost(m_WindowHelper.h_BackBufferPixels));
	//m_WindowHelper.h_BackBufferPixels = nullptr;
...
CPU_CALLABLE void CUDARenderer::Present()
{
	...
	const size_t size = m_WindowHelper.Width * m_WindowHelper.Height * sizeof(*m_WindowHelper.h_BackBufferPixels);
	...
	const FMatrix4& viewProjectionMatrix, const FMatrix4& worldMatrix)
{
	...
	return OVertex{ std::move(NDCspace), std::move(vertex.uv), std::move(worldNormal), std::move(worldTangent), std::move(vertex.c), std::move(viewDirection) };
}
...
GPU_CALLABLE bool DepthTest(FPoint4 rasterCoords[3], float& depthBuffer, float weights[3], float& zInterpolated)
{
	...
	//if (zInterpolated > depthBuffer) return false; //DEPTH BUFFER INVERTED INTERPRETATION
...
GPU_KERNEL void ClearFrameBuffer(unsigned int dev_FrameBuffer[], const unsigned int width, const unsigned int height, RGBColor colour)
{
	//TODO: too many global accesses
	const unsigned int x = threadIdx.x + blockIdx.x * blockDim.x;
	const unsigned int y = threadIdx.y + blockIdx.y * blockDim.y;
	...
	const PrimitiveTopology pt)
{
	...
	else //if (pt == PrimitiveTopology::TriangleStrip)
	{
		if (indexIdx < numIndices - 2)
		{
			...
			const int idx0{ dev_IndexBuffer[indexIdx] };
			const int idx1 = isOdd ? dev_IndexBuffer[indexIdx + 2] : dev_IndexBuffer[indexIdx + 1];
			const int idx2 = isOdd ? dev_IndexBuffer[indexIdx + 1] : dev_IndexBuffer[indexIdx + 2];
...
GPU_KERNEL void RasterizerKernel(Triangle dev_Triangles[], const size_t numTriangles, unsigned int dev_FrameBuffer[], float dev_DepthBuffer[], 
	GPUTextures& textures, SampleState sampleState, bool isDepthColour, const unsigned int width, const unsigned int height)
{
	...
	if (triangleIndex < numTriangles)
	{
		...
		FPoint4 rasterCoords[3]{ triangle.v0.v, triangle.v1.v, triangle.v2.v };
		...
		if (FrustumTest(rasterCoords))
		{
			...
			for (unsigned short r = bb.yMin; r < bb.yMax; ++r)
			{
				for (unsigned short c = bb.xMin; c < bb.xMax; ++c)
				{
					...
					if (IsPixelInTriangle(rasterCoords, pixel, weights))
					{
						...
						//__syncthreads();
						//int formerValue = atomicAdd(lol, 1);
						const unsigned short pixelIdx = c + r * width;
						if (DepthTest(rasterCoords, dev_DepthBuffer[pixelIdx], weights, zInterpolated))
						{
							OVertex oVertex;
							...
							new (&oVertex.v) FPoint4{ pixel, zInterpolated, wInterpolated };
							...
							const char rgba[4] = { (unsigned char)(colour.r / 255.f), (unsigned char)(colour.g / 255.f), (unsigned char)(colour.b / 255.f), UCHAR_MAX };
							memcpy(&dev_FrameBuffer[pixelIdx], rgba, sizeof(unsigned int));
						}
					}
				}
...
CPU_CALLABLE void CUDARenderer::Clear(const RGBColor& colour)
{
	...
	ClearFrameBuffer<<<numBlocks, numThreadsPerBlock>>>
		(dev_FrameBuffer, m_WindowHelper.Width, m_WindowHelper.Height, colour);
}
...
CPU_CALLABLE void CUDARenderer::Rasterizer(GPUTextures& textures, SampleState sampleState, bool isDepthColour)
{
	...
	RasterizerKernel<<<numBlocks, numThreadsPerBlock>>>(
		...
		dev_FrameBuffer, dev_DepthBuffer,
