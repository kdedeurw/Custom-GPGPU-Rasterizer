...
/*=============================================================================*/
// Matrix3.h: Column Major Matrix3x3 struct
/*=============================================================================*/
#pragma once
...
#pragma region Constructors
	BOTH_CALLABLE Matrix<3, 3, T>() 
	{};
	//Every "row" of values passed here is a row in our matrix!
	BOTH_CALLABLE Matrix<3, 3, T>(T _00, T _01, T _02,
		T _10, T _11, T _12,
		T _20, T _21, T _22)
	{
	...
	BOTH_CALLABLE Matrix<3, 3, T>(const Matrix<4, 4, T>& m)
	{
		data[0][0] = m.data[0][0]; data[0][1] = m.data[0][1]; data[0][2] = m.data[0][2];
		data[1][0] = m.data[1][0]; data[1][1] = m.data[1][1]; data[1][2] = m.data[1][2];
		data[2][0] = m.data[2][0]; data[2][1] = m.data[2][1]; data[2][2] = m.data[2][2];
	}
	BOTH_CALLABLE Matrix<3, 3, T>(Matrix<3, 3, T> && m) noexcept
	{
	...
	BOTH_CALLABLE Matrix<3, 3, T>(Matrix<4, 4, T>&& m) noexcept
	{
		data[0][0] = std::move(m.data[0][0]); data[0][1] = std::move(m.data[0][1]); data[0][2] = std::move(m.data[0][2]);
		data[1][0] = std::move(m.data[1][0]); data[1][1] = std::move(m.data[1][1]); data[1][2] = std::move(m.data[1][2]);
		data[2][0] = std::move(m.data[2][0]); data[2][1] = std::move(m.data[2][1]); data[2][2] = std::move(m.data[2][2]);
...
#pragma region ArithmeticOperators
	BOTH_CALLABLE inline Matrix<3, 3, T> operator+(const Matrix<3, 3, T>& m) const
	{ 
		return Matrix<3, 3, T>(
			data[0][0] + m.data[0][0], data[0][1] + m.data[0][1], data[0][2] + m.data[0][2],
			data[1][0] + m.data[1][0], data[1][1] + m.data[1][1], data[1][2] + m.data[1][2],
	...
	template<typename U>
	BOTH_CALLABLE inline Matrix<3, 3, T> operator*(U scale) const
	{
	...
	BOTH_CALLABLE inline Matrix<3, 3, T> operator*(const Matrix<3, 3, T>& rm) const
	{
	...
	BOTH_CALLABLE inline Vector<3, T> operator*(const Vector<3, T>& v) const
	{
	...
	BOTH_CALLABLE inline Point<3, T> operator*(const Point<3, T>& p) const
	{
		const Matrix<3, 3, T>& m = (*this);
		return Point<3, T>(
...
#pragma region CompoundAssignmentOperators
	BOTH_CALLABLE inline Matrix<3, 3, T>& operator=(const Matrix<3, 3, T>& m)
	{ 
		data[0][0] = m.data[0][0]; data[0][1] = m.data[0][1]; data[0][2] = m.data[0][2];
		data[1][0] = m.data[1][0]; data[1][1] = m.data[1][1]; data[1][2] = m.data[1][2];
		data[2][0] = m.data[2][0]; data[2][1] = m.data[2][1]; data[2][2] = m.data[2][2];
	...
	template<typename U>
	BOTH_CALLABLE inline Matrix<3, 3, T>& operator*=(U scale)
	{
		const T s = static_cast<T>(scale);
		data[0][0] *= s; data[0][1] *= s; data[0][2] *= s;
		data[1][0] *= s; data[1][1] *= s; data[1][2] *= s;
...
#pragma region RelationalOperators
	BOTH_CALLABLE inline bool operator==(const Matrix<3, 3, T>& m) const
...
#pragma region MemberAccessOperators
	...
	BOTH_CALLABLE inline const Vector<3, T>& operator[](uint8_t c) const
	{
		assert((c < 3) && "ERROR: index of Matrix3x3 [] operator is out of bounds!");
		return *((Vector<3, T>*)data[c]);
	}

	BOTH_CALLABLE inline Vector<3, T>& operator[](uint8_t c)
	{
		assert((c < 3) && "ERROR: index of Matrix3x3 [] operator is out of bounds!");
		return *((Vector<3, T>*)data[c]);
	}
#pragma endregion
	...
	//=== Static Functions ===
	BOTH_CALLABLE static Matrix<3, 3, T> Identity();
	};
...
template<typename T>
BOTH_CALLABLE inline Matrix<3, 3, T> Matrix<3, 3, T>::Identity()
{
	return Matrix<3, 3, T>(
		1,0,0,
		0,1,0,
...
template<typename T>
BOTH_CALLABLE inline Matrix<3, 3, T> Transpose(const Matrix<3, 3, T>& m)
{
	Matrix<3, 3, T> t = {};
	t(0, 0) = m(0, 0);
	t(0, 1) = m(1, 0);
...
template<typename T>
BOTH_CALLABLE inline T Determinant(const Matrix<3, 3, T>& m)
{
	return 
...
template<typename T>
BOTH_CALLABLE inline Matrix<3, 3, T> Inverse(const Matrix<3, 3, T>& m)
{
...
template<typename T>
BOTH_CALLABLE inline Matrix<3, 3, T> MakeRotationX(T t)
{
...
template<typename T>
BOTH_CALLABLE inline Matrix<3, 3, T> MakeRotationY(T t)
{
	T c = static_cast<T>(cos(t));
	T s = static_cast<T>(sin(t));
...
template<typename T>
BOTH_CALLABLE inline Matrix<3, 3, T> MakeRotationZ(T t)
{
	T c = static_cast<T>(cos(t));
	T s = static_cast<T>(sin(t));
...
template<typename T>
BOTH_CALLABLE inline Matrix<3, 3, T> MakeRotationZYX(T x, T y, T z)
{
...
template<typename T>
BOTH_CALLABLE inline Matrix<3, 3, T> MakeRotation(T t, const Vector<3, T>& axis)
{
	T c = static_cast<T>(cos(t));
	T s = static_cast<T>(sin(t));
...
template<typename T>
BOTH_CALLABLE inline Matrix<3, 3, T> MakeScale(T x, T y, T z)
{
	return Matrix<3, 3, T>(
...
template<typename T>
BOTH_CALLABLE inline Matrix<3, 3, T> MakeScale(T s, const Vector<3, T>& axis)
{
...
template<typename T>
BOTH_CALLABLE inline Matrix<3, 3, T> MakeTranslation(const Vector<2, T>& v)
{
	return Matrix<3, 3, T>(
		1, 0, v.x,
		0, 1, v.y,
...
template<typename T>
BOTH_CALLABLE inline Matrix<3, 3, T> MakeReflection(const Vector<3, T>& axis)
{
	T xy = axis.x * axis.y;
	T yz = axis.y * axis.z;
	T xz = axis.x * axis.z;
